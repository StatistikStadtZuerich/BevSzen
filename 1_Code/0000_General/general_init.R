...
init <- function(i_scen = "middle"){
  ...
  if (!exists("code_path", envir = .GlobalEnv)) {
    ...
    # import parameters
    para <- read_delim(paste0(vars$data_path, "3_Parameter/parameter.csv"), ";", lazy = FALSE) %>%
    ...
    # lookup tables
    vars$look_dis <- read_csv2(paste0(vars$data_path, "2_Lookup/lookupDistrict.csv"), lazy = FALSE) %>%
      select(QuarCd, distr)
