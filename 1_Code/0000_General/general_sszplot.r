...
#' @param data main tibble
#' @param geom a char vector with the names of the desired geom. Default is geom_line.  
#'       if "", then geom must be defined in quotes parameter (see below)
#' @param aes_x the grouping variable for x as string
#' @param aes_y the grouping variable for y as string
#' @param aes_col the grouping variable for colour as string.  
#'          Is converted to a factor if it is continuous
#' @param aes_ltyp the grouping variable for linetype as string
#' @param aes_alpha the alpha value for a geom as string
#' @param aes_size the size value for a geom as string
#' @param labs_x,labs_y,labs_col,labs_ltyp,labs_alpha,labs_size  
#' labels for the respective variables
#' @param i_x vector of vertical intercepts on x axis
#' @param i_y vector of horizontal intercepts on y axis
#' @param scale_x set the x scale. Default ist pretty_breaks for continuous scale
#' @param scale_y set the y scale. Default is none. "pretty" for pretty_breaks, "log"  
#'          for a log10 transformation, a vector for limits.  
#'          Achieve "expand_limits(y = 0)" with c(0, NA)
#' @param breaks vector with breaks for scale if desired.
#' @param fix_col desired length of a colour vector selected from predefined colours  
#'          is ignored (and therefore unnecessary) if aes_col is set
#' @param fix_size desired size of geom (i.e. of line or points)
#'          is ignored (and therefore unnecessary) if aes_size is set
#' @param grid if set, creates a facet grid with grid[1] on LHS
#'       and grid[2] on RHs of formula
#' @param gridscale use to define the scale attribute in facet_grid/facet_wrap
#' @param wrap if set, creates a facet wrap with this value on RHs of formula
#' @param ncol argument cols for facet_grid, ncol for facet_wrap
#' @param title title for the whole chart, as object or string
#'        (esp. in case of multipage plots)
#' @param name if set, stores graphics under this name. otherwise to graphics device
#' @param file_type select file type for storing. Default is pdf
#' @param width width of output file
#' @param height height of output file
#' @param multi apply the plot on this list recursively and
#'         combine the plots on multipage document
#' @param multif run the plot recursively with this expression in tidy format
#'         (usually a filter, e.g.: "filter(year == x)".
#'         Defaults for uni_d/uni_o in multi are handled and can be omitted here
#' @param quotes this argument can be used to hand over any quoted ggplot arguments
...
#' @param quotes_top same as quotes but is inserted at the very top of
#'             the plot building statement
#' @param angle angle of text on y axis (in degrees)
#' 
#' @import dplyr
#' @import ggplot2
#' @import tidyr
...
sszplot <- function(data,
  ...
  if (is.null(multi)) {
    ...
    if (is.null(def_col)) {
      ...
      # due to data masking of dplyr, this is pretty cumbersome to achieve dynamically;
      ...
      if (is.numeric(eval(str2lang(paste0("data$", def_col))))) {
        ...
        if (!is.null(aes_fill)) {
          colour_col <- data %>% select(aes_fill) %>% distinct %>% pull()
          if (!is.factor(colour_col))
            data <- data %>% mutate("{aes_fill}" := as.factor(.data[[aes_fill]]))       
        }
      }
    }
