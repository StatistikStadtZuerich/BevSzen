#' demography housing function
#'
#' @param params parameter vector
#'
#' @returns list with demography housing data
#'      asdf: out_pop: future end year population by district, year, age, sex, origin
#'      out_bir: future births by district, year, age of mother, sex, origin
#'      out_bir_dao: future births by district, year, age of mother, origin of mother (to calculate TFR by origin)
#'      out_dem: future demographic processes except birth (since there by age of mother) by district, year, age, sex, origin
#'      out_nat: future naturalizations by district, year, age, sex
#'      out_bal: future demographic balance (for checks)
#'      out_pop_smooth: future population (for smoothing checks)
#'
#' @export
#'
#' @examples demography_housing_fun(params = params)
#'

demography_housing_fun <- function(params = params){
  ...
  out_path_scen <- params$out_path_scen  
  ...
  # demography housing parameters
  age_min <- params$age_min
  age_max <- params$age_max
  less_ims <- params$less_ims
  more_ims <- params$more_ims
  deh_span <- params$deh_span
  ...
  fer <- read_csv(file.path(exp_path_scen, "birth_fertility_future.csv"),
                  lazy = FALSE) |>
    mutate(
  ...
  # birth: origin change
  cha <- read_csv(file.path(exp_path_scen, "birth_origin-change_future.csv"),
                  lazy = FALSE) |>
    mutate(
      district = factor(district, levels = uni_d),
      origin = factor(origin, levels = uni_o)
  ...
  pro_male <- read_csv(file.path(exp_path_scen, "birth_sex-ratio_future.csv"), 
                       lazy = FALSE)


  # death: mortality rate
  mor <- read_csv(file.path(exp_path_scen, "mortality_future.csv"), 
                  lazy = FALSE) |>
    mutate(sex = factor(sex, levels = uni_s))
  

  # immigration*: immigration* rate
  ims_rate <- read_csv(file.path(exp_path_scen,
                                 "immigration-star_rate-dy_future.csv"), 
                       lazy = FALSE) |>
    mutate(district = factor(district, levels = uni_d))

  
  # immigration*: sex and origin proportion
  ims_prop_so <- read_csv(file.path(exp_path_scen, 
                                    "immigration-star_prop-so-dy_future.csv"), 
                          lazy = FALSE) |>
    mutate(
      district = factor(district, levels = uni_d),
  ...
  ims_prop_a <- read_csv(file.path(exp_path_scen,
  ...
  ems_rate <- read_csv(file.path(exp_path_scen,
                                 "emigration-star_rate-dy_future.csv"), 
                       lazy = FALSE) |>
    mutate(district = factor(district, levels = uni_d))

  
  # emigration*: sex and origin proportion
  ems_prop_so <- read_csv(file.path(exp_path_scen,
                                    "emigration-star_prop-so-dy_future.csv"),
                          lazy = FALSE) |>
    mutate(
      district = factor(district, levels = uni_d),
      sex = factor(sex, levels = uni_s),
  ...
  ems_prop_a <- read_csv(file.path(exp_path_scen,
  ...
  rei <- read_csv(file.path(exp_path_scen,
  ...
  ree <- read_csv(file.path(exp_path_scen,
  ...
  # naturalization
  nat <- read_csv(file.path(exp_path_scen,
                            "naturalization_future.csv"),
                  lazy = FALSE) |>
    mutate(
      district = factor(district, levels = uni_d),
      sex = factor(sex, levels = uni_s),
  ...
  # housing model
  hou <- read_csv(file.path(exp_path_scen, 
                            "housing-model_population_d.csv"), 
                  lazy = FALSE) |>
    mutate(district = factor(district, levels = uni_d)) |>
  ...
  # outputs
  out_bir <- NULL
  out_bir_dao <- NULL # origin of the mother (to calculate TFR by origin)
  out_dem <- NULL
  out_pop <- NULL
  out_nat <- NULL
  ...
  for (iyear in future) {
    ...
    # outputs: birth with origin of mother (to calculate TFR by origin)
    out_bir_dao <- bir_dao |> 
      mutate(year = iyear) |>      
      rename(origin_mother = origin, 
             age_mother = age) |> 
      select(district, year, age_mother, origin_mother, bir) |> 
      bind_rows(out_bir_dao)
  ...
  out_bal |>
    ...
    write_csv(file.path(out_path_scen, "balance_Wollishofen.csv"))
  ...
  out_bir |>
    ...
    write_csv(file.path(out_path_scen, "births_future.csv"))
  
  # births with origin of the mother
  out_bir_dao |>
    arrange(district, year, age_mother, origin_mother) |>
    write_csv(file.path(out_path_scen, "births_dao_future.csv"))  

  # demographic processes
  out_dem |>
    select(district, year, age, sex, origin, dea, ims, imm, rei, ems, emi, ree) |>
    arrange(district, year, age, sex, origin) |>
    write_csv(file.path(out_path_scen, "demographic-processes_future.csv"))

  # population (end of year)
  out_pop |>
    arrange(district, year, age, sex, origin) |>
    write_csv(file.path(out_path_scen, "population_future.csv"))

  # naturalization
  out_nat |>
    arrange(district, year, age, sex) |>
    write_csv(file.path(out_path_scen, "naturalization_future.csv"))

  # log info
  cat_log(
    text = paste0("demography and housing: ", capture.output(Sys.time() - t0)),
    log_file = log_file
  )    
  ...
  out$out_pop <- out_pop
  out$out_bir <- out_bir  
  out$out_bir_dao <- out_bir_dao
  out$out_dem <- out_dem  
  out$out_nat <- out_nat  
  out$out_bal <- out_bal    
  out$out_pop_smooth <- out_pop_smooth  
  return(out)  
