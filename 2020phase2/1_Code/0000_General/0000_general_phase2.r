...
#import parameter
    para <- read_delim("2_Data/3_Parameter/parameter.csv", ";") %>% 
        select(parameter, value)

#assign values to parameters
    for (ipara in 1:nrow(para)) {assign(para$parameter[ipara], para$value[ipara])}
...
#districts (34 numbers)
    ...
    look_dis <- read_csv2("2_Data/2_Lookup/lookupDistrict.csv") %>% 
        select(QuarCd, distr)
        ...
        mutate(age_2 = factor(if_else(age < age_2[1], age_2t[1],
                if_else(age < age_2[2], age_2t[2],
                    if_else(age < age_2[3], age_2t[3],
                        if_else(age < age_2[4], age_2t[4], age_2t[5])))), levels = age_2t))    
...
#rok: url-function = trouble maker, therefore without url-function                 
...
#plot function
    source("1_Code/0000_General/0001_plot-functions.r")    
    
#constrained regression
    source("1_Code/0000_General/0011_constrained-regression.r")

#life expectancy
    source("1_Code/0000_General/0012_life-expectancy.r")   
    
#migration functions
    source("1_Code/0000_General/0013_migration-functions.r")
    
#relocation functions
    source("1_Code/0000_General/0014_relocation-function.r")
