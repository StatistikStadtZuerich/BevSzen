...
# import, data preparation ------------------------------------------------
  ...
  pop <- as_tibble(expand_grid(district = uni_d,
...
# Loop over years ---------------------------------------------------------
  ...
  for (iyear in future) {
    ...
    dem <- as_tibble(expand_grid(district = uni_d,
                                 year = iyear,
                                 age = (-1):age_max,
                                 sex = uni_s,
                                 origin = uni_o)) %>%
    ...
    bal <- dem %>%
      ...
      summarize(pop_bir_dea = sum(pop_bir_dea),
                ims = sum(ims),
                ems = sum(ems)) %>%
      ungroup() %>%
    ...
    pop_nat <- nat %>%
      ...
      pivot_longer(cols = all_of(uni_o),
...
# ex. function ------------------------------------------------------------
# demo function for lines 310-317, may be deleted if not desired
  in_decr <- function(differ, ms, emm = FALSE) {
    differ_ms <- if_else(differ < 0,
                         less_ims / 100 * differ,
                         more_ims / 100 * differ)
    if (emm) differ_ms <- differ_ms - differ
