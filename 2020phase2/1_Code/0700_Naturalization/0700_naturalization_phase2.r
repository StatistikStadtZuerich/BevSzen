...
#plot preparation
    ...
    temp_initial <- gather(nat_pop_das_smooth, `nat`, `pop`, key = category, value = count) %>% 
        mutate(cat = factor(if_else(category == "nat", 
    ...
    temp_smooth <- gather(nat_pop_das_smooth, `nat_a`, `pop_a`, key = category, value = count) %>% 
        mutate(cat = factor(if_else(category == "nat_a", 
            process_lev[1], process_lev[2]), levels = process_lev),
...
#prediction
    rate_ya_pred <- con_reg(data = nat_rate_ya, x = "year", y = "rate_smooth",
              group_cols = "age",
...
#preparation
    prep_rate_ya <- filter(rate_ya_past_pred, year >= szen_begin) %>% 
